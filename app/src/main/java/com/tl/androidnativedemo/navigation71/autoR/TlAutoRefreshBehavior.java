...
import com.tl.androidnativedemo.navigation71.pullrefresh.TLRefresh;
...
public class TlAutoRefreshBehavior extends CoordinatorLayout.Behavior<View> implements TLRefresh
{
    /**
    ...
    public TlAutoRefreshBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
...
//         Log.d("my", "dyConsumed = " + dyConsumed + ", dyUnconsumed = " + dyUnconsumed + ", type = " + type);
        if (mode == MODE_READY) {
            return;
        }
        if (isAnimRunning)
        ...
        if (dyConsumed == 0) {
            ...
            if (dyUnconsumed > 0 && type == 1 && first != 0) {
                if (!canLoad) {
                    canLoad = true;
    ...
    public void onStopNestedScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull View child, @NonNull View target, int type) {
        ...
        if (isAnimRunning)
            return;
        Log.d("my", "onStopNestedScroll load---" + canLoad);
